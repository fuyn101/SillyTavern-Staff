...
check_quota() {
    ...
    quota_output=$(gcloud services quota list --service=cloudresourcemanager.googleapis.com --consumer="projects/$(gcloud config get-value project)" --filter="metric.value(name.format()):project_create_requests" --format="value(consumerQuotaLimits.value.effectiveLimit,metric.value.values.currentValue)" 2>/dev/null)
    local limit usage; read -r limit usage <<< "$quota_output"
    if [[ ! "$limit" =~ ^[0-9]+$ ]]; then
        log "WARN" "无法自动获取配额信息，将跳过检查。"; return 0
    fi
    local remaining=$((limit - usage)); log "INFO" "当前项目创建配额: 总计 $limit, 已用 $usage, 剩余 $remaining"
    if (( TOTAL_PROJECTS > remaining )); then
        log "WARN" "计划创建的项目数 ($TOTAL_PROJECTS) 超出剩余配额 ($remaining)!"; read -p "是否自动将创建数量调整为 $remaining? [Y/n]: " r
        if [[ ! "$r" =~ ^[Nn]$ ]]; then TOTAL_PROJECTS=$remaining; log "INFO" "项目创建数量已调整为: $TOTAL_PROJECTS"; else log "WARN" "将按原计划 ($TOTAL_PROJECTS) 继续。"; fi
    fi
    if (( TOTAL_PROJECTS <= 0 )); then log "ERROR" "没有足够的配额来创建任何新项目。"; return 1; fi; return 0
}
...
create_projects_phased() {
    ...
    while true; do
        ...
        log "INFO" "阶段2: 全局等待 ${GLOBAL_WAIT_SECONDS} 秒..."
...
extract_keys_from_existing_projects() {
    ...
    for project_id in "${projects_array[@]}"; do
        # 使用list_existing_api_keys函数来检查密钥状态，它会处理权限错误
        if ! list_existing_api_keys "$project_id" >/dev/null 2>&1; then
            # 如果函数返回失败（没有密钥或没有权限），则添加到处理列表
...
configure_settings() {
  while true; do
      ...
      echo " 4. 全局等待时间(秒)   : $GLOBAL_WAIT_SECONDS"; echo " 5. 最大重试次数       : $MAX_RETRY_ATTEMPTS"
      ...
      case $choice in
          ...
          4) read -p "输入全局等待时间 (秒, >=30): " val; if [[ "$val" =~ ^[0-9]+$ && "$val" -ge 30 ]]; then GLOBAL_WAIT_SECONDS=$val; log "INFO" "配置更新: 全局等待 -> $GLOBAL_WAIT_SECONDS"; fi;;
