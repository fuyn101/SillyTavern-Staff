          ...
          <n-card title="核心信息" :bordered="false" header-style="padding-bottom: 8px;">
            <n-grid :cols="12" :x-gap="24">
              <n-form-item-gi :span="4" label="名称" path="name">
                <n-input v-model:value="editorCard.name" placeholder="角色名称" />
              </n-form-item-gi>
              <n-form-item-gi :span="8" label="标签" path="tags">
                <n-dynamic-tags v-model:value="editorCard.tags" />
              </n-form-item-gi>
              <n-form-item-gi :span="12" label="(角色)描述" path="description">
                <n-input
                  v-model:value="editorCard.description"
                  type="textarea"
                  :autosize="{ minRows: 3 }"
              ...
              <n-form-item-gi :span="12" label="开场白" path="first_mes">
                <n-input
                  v-model:value="editorCard.first_mes"
                  type="textarea"
                  :autosize="{ minRows: 3 }"
          ...
          <n-card title="角色扮演" :bordered="false" header-style="padding-bottom: 8px;">
            <n-grid :cols="12" :x-gap="24">
              <n-form-item-gi :span="6" label="个性" path="personality">
                <n-input
                  v-model:value="editorCard.personality"
                  type="textarea"
                  :autosize="{ minRows: 2 }"
                  placeholder="角色的性格特点。"
                />
              ...
              <n-form-item-gi :span="6" label="场景" path="scenario">
                <n-input
                  v-model:value="editorCard.scenario"
                  type="textarea"
                  :autosize="{ minRows: 2 }"
              ...
              <n-form-item-gi :span="12" label="对话示例" path="mes_example">
                <n-input
                  v-model:value="editorCard.mes_example"
                  type="textarea"
                  :autosize="{ minRows: 4 }"
          ...
          <n-card title="元数据" :bordered="false" header-style="padding-bottom: 8px;">
            <n-grid :cols="12" :x-gap="24" :y-gap="8">
              <n-form-item-gi :span="4" label="头像" path="avatar">
                <n-input v-model:value="editorCard.avatar" placeholder="avatar.png" />
              </n-form-item-gi>
              <n-form-item-gi :span="4" label="聊天记录" path="chat">
                <n-input v-model:value="editorCard.chat" placeholder="char_chat.jsonl" />
              </n-form-item-gi>
              ...
              <n-form-item-gi :span="4" label="创建日期" path="create_date">
                <n-input v-model:value="editorCard.create_date" />
              </n-form-item-gi>
              <n-form-item-gi :span="4" label="规格" path="spec">
                <n-input v-model:value="editorCard.spec" readonly />
              </n-form-item-gi>
              <n-form-item-gi :span="4" label="规格版本" path="spec_version">
                <n-input v-model:value="editorCard.spec_version" readonly />
              </n-form-item-gi>
              <n-form-item-gi :span="4" label="收藏" path="fav">
                <n-switch v-model:value="editorCard.fav" />
              </n-form-item-gi>
              <n-form-item-gi :span="8" label="创建者备注" path="creatorcomment">
                <n-input
                  v-model:value="editorCard.creatorcomment"
                  type="textarea"
                  :autosize="{ minRows: 1 }"
                />
              </n-form-item-gi>
      ...
      >
        <n-space vertical size="large">
            ...
            <n-space vertical size="large">
              ...
              <n-grid :cols="12" :x-gap="24" :y-gap="8">
                <n-form-item-gi :span="4" label="角色卡名称" path="name">
                  <n-input v-model:value="editorCard.data.name" placeholder="请输入角色卡名称" />
                </n-form-item-gi>
                <n-form-item-gi :span="4" label="创作者" path="creator">
                  <n-input v-model:value="editorCard.data.creator" placeholder="请输入创建者" />
                </n-form-item-gi>
                <n-form-item-gi :span="4" label="角色卡版本号" path="character_version">
                  <n-input v-model:value="editorCard.data.character_version" placeholder="请输入角色版本" />
                </n-form-item-gi>
                <n-form-item-gi :span="12" label="标签" path="tags">
                  <n-dynamic-tags v-model:value="editorCard.data.tags">
              ...
              <n-grid :cols="12" :x-gap="24" :y-gap="8">
                <n-form-item-gi :span="12" label="数据描述" path="description">
                  <n-input v-model:value="editorCard.data.description" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
                <n-form-item-gi :span="6" label="角色性格" path="personality">
                  <n-input v-model:value="editorCard.data.personality" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
                <n-form-item-gi :span="6" label="角色场景" path="scenario">
                  <n-input v-model:value="editorCard.data.scenario" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
                <n-form-item-gi :span="12" label="角色对话示例" path="mes_example">
                  <n-input v-model:value="editorCard.data.mes_example" type="textarea" :autosize="{ minRows: 3 }" />
                </n-form-item-gi>
              </n-grid>
              ...
              <n-grid :cols="12" :x-gap="24" :y-gap="8">
                <n-form-item-gi :span="12" label="角色开场白" path="first_mes">
                  <n-input v-model:value="editorCard.data.first_mes" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
                <n-form-item-gi :span="12" label="备用开场白" path="alternate_greetings">
                  <n-dynamic-input
                    v-model:value="editorCard.data.alternate_greetings"
              ...
              <n-grid :cols="12" :x-gap="24" :y-gap="8">
                <n-form-item-gi :span="12" label="创作者备注" path="creator_notes">
                  <n-input v-model:value="editorCard.data.creator_notes" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
                <n-form-item-gi :span="12" label="系统提示词" path="system_prompt">
                  <n-input v-model:value="editorCard.data.system_prompt" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
                <n-form-item-gi :span="12" label="聊天后指令（又称后置指令）" path="post_history_instructions">
                  <n-input v-model:value="editorCard.data.post_history_instructions" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
              </n-grid>
            </n-space>
          </n-card>
          ...
          <n-card title="拓展用字段" :bordered="false" header-style="padding-bottom: 8px;">
            <n-space v-if="editorCard.data.extensions" vertical size="large">
              <!-- 通用拓展 -->
              ...
              <n-grid :cols="12" :x-gap="24" :y-gap="8">
                ...
                <n-form-item-gi :span="3" label="世界" path="extensions.world">
                  <n-input v-model:value="editorCard.data.extensions.world" placeholder="世界名称" />
                </n-form-item-gi>
                <n-form-item-gi :span="3" label="收藏" path="extensions.fav">
                  <n-switch v-model:value="editorCard.data.extensions.fav" />
                </n-form-item-gi>
              </n-grid>
              ...
              <n-grid :cols="12" :x-gap="24" :y-gap="8">
                <n-form-item-gi :span="3" label="深度" path="extensions.depth_prompt.depth">
                  <n-input-number v-model:value="editorCard.data.extensions.depth_prompt.depth" :min="1" :max="10" />
                </n-form-item-gi>
                <n-form-item-gi :span="9" label="角色" path="extensions.depth_prompt.role">
                  <n-input v-model:value="editorCard.data.extensions.depth_prompt.role" placeholder="角色" />
                </n-form-item-gi>
                <n-form-item-gi :span="12" label="提示内容" path="extensions.depth_prompt.prompt">
                  <n-input v-model:value="editorCard.data.extensions.depth_prompt.prompt" type="textarea" :autosize="{ minRows: 2 }" />
                </n-form-item-gi>
              </n-grid>
              ...
              <n-dynamic-input
                v-model:value="editorCard.data.extensions.regex_scripts"
                :on-create="handleCreateRegexScript"
                v-if="editorCard.data.extensions"
                #="{ value: script, index }"
              >
          ...
          <n-card title="角色书 / 世界观设定" :bordered="false" header-style="padding-bottom: 8px;">
            <n-space v-if="editorCard.data.character_book" vertical size="large">
              <n-form-item label="世界书名称" path="character_book.name">
                <n-input v-model:value="editorCard.data.character_book.name" placeholder="请输入世界书名称" />
              </n-form-item>

              <n-h4 prefix="bar">条目</n-h4>
              <n-dynamic-input
                v-model:value="editorCard.data.character_book.entries"
...
const talkativenessNumber = computed(() => {
  const stringValue = editorCard.value.talkativeness;
  return typeof stringValue === 'string' ? parseFloat(stringValue) || 0.5 : stringValue || 0.5;
});

const updateTalkativeness = (value: number | null) => {
  editorCard.value.talkativeness = value === null ? '0.5' : value.toString();
};

// --- Data Editor Logic ---
onMounted(() => {
  if (!editorCard.value.data.extensions) {
    editorCard.value.data.extensions = {
      talkativeness: '0.5',
      fav: false,
      world: '',
      depth_prompt: {
  ...
  if (!editorCard.value.data.character_book) {
    editorCard.value.data.character_book = {
      name: '',
      entries: []
    };
  }
...
const extensionsTalkativenessNumber = computed(() => {
  const stringValue = editorCard.value.data.extensions?.talkativeness;
  return typeof stringValue === 'string' ? parseFloat(stringValue) || 0.5 : stringValue || 0.5;
});

const updateExtensionsTalkativeness = (value: number | null) => {
  if (editorCard.value.data.extensions) {
    editorCard.value.data.extensions.talkativeness = value === null ? '0.5' : value.toString();
  }
};
